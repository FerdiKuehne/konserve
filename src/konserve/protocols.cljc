...
(defprotocol PEDNAsyncKeyValueStore
    ...
    (-update-in
      [this key-vec meta-up-fn up-fn]
      [this key-vec meta-up-fn up-fn up-fn-args]
    ...
    (-assoc-in
      [this key-vec meta-up-fn val]
      [this key-vec meta-up-fn val args]) 
    (-dissoc [this key]))
...
(defprotocol PJSONAsyncKeyValueStore
  ...
  (-jassoc-in [this key-vec velektroalue] "Associates the key-vec to the value, any missing collections for the key-vec (nested maps and vectors) are newly created.")
...
(defprotocol PMetaData
  "Allows lazy iteration of keys in this store."
  (-metas [this] "Return a set that yields all metadata of the store."))
