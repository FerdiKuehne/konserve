...
(defn meta-update [key type old]
  (if (empty? old)
    (clojure.core/assoc {:key key :type type} ::timestamp (java.util.Date.))
...
;TODO write format in meta data
...
(defn assoc-in
  ...
  (go-locked
   store (first key-vec)
   (<! (-update-in store key-vec (partial meta-update (first key-vec) :edn) (fn [_] val) args))))
...
(defn dissoc
  ...
  (go-locked
   ...
   (<! (-dissoc store key))))
                                        ...
                                        ;TODO update append to new funktions (update-in)
;TODO dd meta-data everywhere
(defn append
  ...
  (go-locked
   ...
   (let [head (<! (-get store [key]))
     ...
     (<! (-update-in store [id] (fn [_]) (fn [_] new-elem)))
     (when first-id
       (<! (-update-in store [last-id :next] (fn [_]) (fn [_] id))))
     (<! (-update-in store [key] (fn [_]) (fn [_] [:append-log id (or first-id id)])))
     [first-id id])))
...
(defn log
  ...
  [store key]
  (go
   (let [head (<! (get store key));; atomic read
         [append-log? last-id first-id] head] 
